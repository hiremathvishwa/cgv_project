...
void  keyInput(unsigned char key , int x, int y){
    ...
    //
        ...
        break;
        //r for restart
        case 'r' : flagOver =0; glutPostRedisplay();break;
    }
}
...
void draw_circle(double theta, double inner_radius, double outer_radius, int x, int y, int sin_sign = 1, int cos_sign = 1){
   glBegin(GL_POINTS);
   glColor3f(1.0/ 255.0, 0.0/ 255.0, 0.0/ 255.0);
...
void render( void ){
    ...
    glPointSize(2);
    glBegin(GL_POINTS);
        glColor3f((1) / 255.0, (0) / 255.0, (0) / 255.0);
        for(int i = 0; i < 100; i++){
    ...
    if (flagOver == 1)
    {
        ...
        if (isTextVisible)
        {
            ...
            glRasterPos2f(1000, 1000); // Set text position (x, y)
...
void updateScene(int value) {
    if (flagOver == 1) {
        isTextVisible = !isTextVisible; // Toggle the visibility of the text
        glutPostRedisplay();            // Request a redraw of the scene
        // Setup the next update
        glutTimerFunc(500, updateScene, 0);
    } else {
        static int speed = 10; // Adjust the speed value to control the movement speed
        x_ -= speed;
        if (x_ < 0) {
            x_ = 2000 + rand() % 400;
        }
        ...
        glutTimerFunc(16, updateScene, 0); // Adjust the timeout value to control the update rate
        glutPostRedisplay();
    }
}
